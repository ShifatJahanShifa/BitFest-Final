  ...
  return (
    <div className="chat-container">
      {chatHistory.map((message, index) => {
        console.log("messages"+message.type);
        ...
        return (
          ...
          >
            ...
            {/* Message */}
            <div className={`inline-flex ${isUser ? 'bg-gray-100 text-black p-2 rounded-bl-xl mr-[8px] text-right max-w-[1000px] ' : 'max-w-[1000px] text-left ml-[8px] max-w-[800px] bg-gradient-to-r p-2 from-blue-300 via-blue-400 to-violet-400 text-black rounded-br-xl'}`}>
              <p className="text-sm font-normal py-1.5 text-gray-900">
                {message.type === "user" && message.message}
                {message.type === "bot" && <ReactMarkdown>{message.message}</ReactMarkdown>}
              </p>
            </div>
            { isUser &&
